...
function initFilters() {
  ...
  // Preselect a genre or platform if arriving from a link like games.html?genre=Puzzle
  const params = new URLSearchParams(window.location.search);
  const presetGenre = params.get("genre");
  const presetPlatform = params.get("platform");
  if (presetGenre && genres.includes(presetGenre)) genreSelect.value = presetGenre;
  if (presetPlatform && platforms.includes(presetPlatform)) platformSelect.value = presetPlatform;
...
function initGenres() {
  ...
  grid.innerHTML = genres.map((genre) => `
    <a class="category-card" href="games.html?genre=${encodeURIComponent(genre)}">
      <span class="cat-icon">${GENRE_ICONS[genre] || "★"}</span>
      <h3>${genre}</h3>
      <span class="cat-count">${counts[genre]} title${counts[genre] > 1 ? "s" : ""}</span>
    </a>
  `).join("");
}
...
/* ---------- Forms that redirect to thankyou.html ---------- */
function initRedirectForm(formId) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  initDetailPage();
  initFaq();
  initRedirectForm("contact-form");
  initRedirectForm("submit-form");
});
